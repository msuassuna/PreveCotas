...
prev_CRC_36 <- array(NA, nrow(Dados_List))
...
for(i in seq_along(Dados_List$DataHora)){
  if(i > 36){
    prev_CRC_36[i] <- Prev_CARACARAI_36_1(Dados_List$Nivel.x[i],
...
limites <- range(c(Dados_List$Nivel.x, 900,
                   tail(prev_CRC_36[complete.cases(prev_CRC_36)],1)),
                   na.rm = TRUE)


png(paste0("previsoes/",format(Sys.Date(),"%y%m%d"),"_previsao_",14710000,".png"),
...
points(ultimo+36*3600, tail(prev_CRC_36[complete.cases(prev_CRC_36)],1), pch = 20, cex = 2)

abline(h = 900, col = "red", lty = 2)
abline(h = 850, col = "orange", lty = 2)
abline(h = 800, col = "yellow", lty = 2)

legend(ultimo-Dias_eixo_x*24*3600,
       limites[2]+80,
       legend = c("Dados observados", "Previsão 36 horas",
...
mtext("Previsão de níveis em Caracaraí", side = 3, font = 2, line = 0.5)

dev.off()


plot(Dados_List$DataHora, Dados_List$Nivel.x, type = "l", bty = "n",
     xlim = c(ultimo-Dias_eixo_x*24*3600, ultimo+36*3600), ylim = c(limites[1]-80,limites[2]+80))
points(ultimo+36*3600, tail(prev_CRC_36[complete.cases(prev_CRC_36)],1), col = 2, pch = 20, cex = 3)

lines(Dados_List$DataHora+36*3600, prev_CRC_36, col = 2, pch = 20)
