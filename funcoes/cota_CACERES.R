...
Dia <- str_pad(as.character(as.numeric(DiaHoje)-10), 3, pad = "0")

EstHoje_10 <- Est[Est$Dia_Ano == Dia,]
...
points(Sub_Prev$Dia_Ano[c(which(1:H %% 7 == 0)+1)],
       rowMeans(Prev)[c(which(1:H %% 7 == 0)+1)], col = 2, pch = 20, cex = 1)

dev.off()

Prev_Caceres <- rowMeans(Prev, na.rm = TRUE)[c(8,15,22,29)]
